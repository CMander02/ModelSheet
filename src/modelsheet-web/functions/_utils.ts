export interface D1Result<T = Record<string, unknown>> {
  results?: T[]
  meta?: { duration?: number; rows_read?: number; served_by_region?: string; served_by_primary?: boolean }
}
...
export interface D1DatabaseLike {
  ...
  withSession?(constraint: "first-unconstrained"): D1DatabaseLike
}

export function readDatabase(env: FunctionEnv): D1DatabaseLike | undefined {
  return env.DB?.withSession?.("first-unconstrained") ?? env.DB
}
