...
export default function Qwen3Diagram(p: DiagramParams) {
  resetIds()
  const { numLayers = 36, numHeads = 32, numKvHeads = 8, hiddenSize = 4096 } = p

  const input = pill("Input tokens")
  const emb = rect("Token Embedding", "emb")
  const ln1 = rect("RMSNorm", "norm", { sublabel: "pre-norm" })
  const qproj = rect("Q/K/V Proj", "attn", { sublabel: "bias=True" })
  const qknorm = rect("QK-RMSNorm", "norm", { sublabel: "per-head · before RoPE" })
  const attn = rect(`GQA Q:${numHeads} KV:${numKvHeads}`, "attn", { sublabel: "RoPE · Scaled Dot-Product" })
  const r1 = resid()
  ...
  const info = note(`hidden: ${hiddenSize.toLocaleString()} · per-head QK-RMSNorm`)

  const main = seq(input, emb, ln1, qproj, qknorm, attn, r1, ln2, ffn, r2, lnf, lmh)
