...
export default function Glm4Diagram(p: DiagramParams) {
  const { numHeads = 32, numKvHeads = 2, numLayers = 40, hiddenSize = 4096 } = p

  const nodes: TreeNode[] = [
    { id: "input", type: "leaf", label: "Input tokens", color: "input" },
    { id: "emb", type: "leaf", label: "Token Embedding", sub: "no learned pos emb", color: "emb" },
    {
      ...
      children: [
        ...
        { id: "attn", type: "leaf", label: `GQA  Q:${numHeads}  KV:${numKvHeads}`, sub: "RoPE-2D · QKV fused", color: "attn" },
    ...
    { id: "lnf", type: "leaf", label: "Final RMSNorm", sub: "encoder.final_layernorm", color: "norm" },
  ...
  return (
    <ArchTreeDiagram
      nodes={nodes}
      subtitle={`hidden: ${hiddenSize.toLocaleString()} · rope_ratio: 500`}
    />
  )
}
